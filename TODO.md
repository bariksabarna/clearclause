...
- [x] Deploy (Render) — ready via `render.yaml` blueprint (free tier web service, `GEMINI_API_KEY` sync:false so the secret never enters git); steps documented in README "Deployment (Render — recommended)". Remaining step is purely user-driven: create/connect a Render account + set the key, then `SMOKE_BASE_URL=https://<service-url> npm run smoke` verifies live (previous Cloud Run attempt stays blocked on closed GCP billing accounts)
...
## Internal code-quality audit (post-completion)

Two independent read-only audits of `server/` and `client/` surfaced real defects; fixes landed with tests:
